    ...
    code: function(cache) {
        ...
        switch(info) {
            ...
            case 36:
                ...
                break;
            case 37:
                server.fetchBans(true).then(bans => {
                    this.StoreOutputValue(bans.array(), "result", cache);
                    this.RunNextBlock("action", cache);
                    return;
                });
            case 38:
                server.fetchInvites().then(invites => {
                    this.StoreOutputValue(invites.array(), "result", cache);
                    this.RunNextBlock("action", cache);
                    return;
                });
            case 39:
                server.fetchVanityCode().then(code => {
                    this.StoreOutputValue(code, "result", cache);
                    this.RunNextBlock("action", cache);
                    return;
                });
            case 40:
                server.fetchVanityCode().then(code => {
                    this.StoreOutputValue(`https://discord.gg/${code}`, "result", cache);
                    this.RunNextBlock("action", cache);
                    return;
                });
            case 41:
                server.fetchWebhooks().then(webhooks => {
                    this.StoreOutputValue(webhooks.array(), "result", cache);
                    this.RunNextBlock("action", cache);
                    return;
                });
        }
